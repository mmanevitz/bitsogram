...
class config:
    ...
    def incrementArray(arrayOfCounters):
        ...
        for i in range(len(arrayOfCounters) - 1, 0, -1):
            if arrayOfCounters[i] == 26:
                arrayOfCounters[i] = 0
                arrayOfCounters[i - 1] += 1

    @staticmethod
    def genEnglishNgrams(N):
        gramDict = [''] * (int(math.pow(26, N)))
        counter = 0
    ...
    n = 10000000      # Number of data samples, needed only for simulation
    ...
    numNgrams = 3   # Number of N-grams
    gramLength = 2  # Gram length
    emptyChar = '?'  # Character to fill the word with if words are smaller
    threshold = 15.0 * int(math.sqrt(n)) # Threshold for discoverability

    numOfRunsPerDataFile = 10    # Number of experiments to run per file

    dataPath = 'C:\Users\miriam.manevitz\PycharmProjects\codes\codes\dataLoc'
